...
public partial class AutoProfiles : UserControl
{
    ...
    public AutoProfiles()
    {
        ...
        if (currentRowCount > IControlService.CURRENT_DS4_CONTROLLER_LIMIT)
        {
            for (int i = currentRowCount-1; i >= IControlService.CURRENT_DS4_CONTROLLER_LIMIT; i--)
            {
    ...
    {
        autoProfVM = new AutoProfilesViewModel().WithAutoProfileHolder(autoProfileHolder).WithProfileList(profileList).Init();
    ...
    private void RemoveAutoBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            editControlsPanel.DataContext = null;
            autoProfVM.AddExeToHIDHideWhenSaving(autoProfVM.SelectedItem, false);// set to False, to remove the EXE from HIDHide

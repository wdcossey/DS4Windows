...
public class MappedControl
{
    ...
    {
        string temp = Localization.Unassigned;
        ...
        if (actionType != DS4ControlSettings.ActionType.Default)
        {
            ...
            {
                //return (Keys)int.Parse(action.ToString()) + (sc ? " (" + Localization.ScanCode + ")" : "");
                temp = KeyInterop.KeyFromVirtualKey(action.actionKey) + (sc ? " (" + Localization.ScanCode + ")" : "");
            }
            else if (actionType == DS4ControlSettings.ActionType.Macro)
            {
                temp = Localization.Macro + (sc ? " (" + Localization.ScanCode + ")" : "");
            }
    ...
    private static string ShiftTrigger(int trigger)
    {
        switch (trigger)
        {
            ...
            case 22: return Localization.TiltUp;
            case 23: return Localization.TiltDown;
            case 24: return Localization.TiltLeft;
            case 25: return Localization.TiltRight;

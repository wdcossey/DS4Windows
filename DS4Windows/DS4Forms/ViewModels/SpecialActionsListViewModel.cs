    ...
    public string GetActionDisplayName(SpecialAction action)
    {
        ...
        switch (action.typeID)
        {
            case SpecialAction.ActionTypeId.DisconnectBT:
                displayName = Localization.DisconnectBT; break;
            case SpecialAction.ActionTypeId.Macro:
                displayName = Localization.Macro + (action.keyType.HasFlag(DS4KeyType.ScanCode) ? " (" + Localization.ScanCode + ")" : "");
                break;
            case SpecialAction.ActionTypeId.Program:
                displayName = Localization.LaunchProgram.Replace("*program*", Path.GetFileNameWithoutExtension(action.details));
                break;
            case SpecialAction.ActionTypeId.Profile:
                displayName = Localization.LoadProfile.Replace("*profile*", action.details);
                break;

...
{
    private Dictionary<Button, BindAssociation> associatedBindings = new();
    private Dictionary<int, Button> keyBtnMap = new();
    private Dictionary<X360Controls, Button> conBtnMap = new();
    private Dictionary<X360Controls, Button> mouseBtnMap = new();
    ...
    public BindingWindow(IControlService controlService, IBindingWindowViewModelFactory viewModelFactory, int deviceNum, DS4ControlSettings settings,
        ExposeMode expose = ExposeMode.Full)
    {
        ...
        this.expose = expose;
        bindingVM = viewModelFactory.Create(deviceNum, settings);
    ...
    private string GetControlString(Button button)
    {
        ...
        if (bindingVM.Using360Mode)
        {
            X360Controls xboxcontrol = associatedBindings[button].control;
    ...
    {
        OutBinding binding = bindingVM.ActionBinding;
        X360Controls defaultControl = Global.defaultButtonMapping[(int)binding.input];
        Button button = sender as Button;
        if (associatedBindings.TryGetValue(button, out BindAssociation bind))
        {
    ...
    private void FindCurrentHighlightButton()
    {
        ...
        {
            X360Controls defaultBind = Global.defaultButtonMapping[(int)binding.input];
    ...
    private void InitButtonBindings()
    {
        ...
        associatedBindings.Add(touchpadClickBtn,
            new BindAssociation() { outputType = BindAssociation.OutType.Button, control = X360Controls.TouchpadClick });
        ...
        associatedBindings.Add(mouseMBBtn,
            new BindAssociation() { outputType = BindAssociation.OutType.Button, control = X360Controls.MiddleMouse });
        ...
        associatedBindings.Add(mouse4Btn,
            new BindAssociation() { outputType = BindAssociation.OutType.Button, control = X360Controls.FourthMouse });
    ...
    private void InitDS4Canvas()
    {
        ...
        ImageSource temp = sourceConverter.
            ConvertFromString($"{Global.ASSEMBLY_RESOURCE_PREFIX}component/Resources/{Application.Current.FindResource("DS4ConfigImg")}") as ImageSource;
        ...
        if (deviceNum < IControlService.CURRENT_DS4_CONTROLLER_LIMIT)
        {
            ...
            if (d != null)
            {
                if (!bindingVM.RumbleActive)
                {
                    ...
                    testRumbleBtn.Content = Localization.StopText;
                }
                else
                {
                    bindingVM.RumbleActive = false;
                    d.SetRumble(0, 0);
                    testRumbleBtn.Content = Localization.TestText;
                }
            }
        }
    }

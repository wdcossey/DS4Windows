...
public class AutoProfileChecker
{
    ...
    private StringBuilder autoProfileCheckTextBuilder = new(1000);
    ...
    public AutoProfileChecker(IControlService controlService, AutoProfileHolder holder)
    {
    ...
    public void Process()
    {
        ...
        if (GetTopWindowName(out topProcessName, out topWindowTitle))
        {
            ...
            if (matchedProfileEntity != null)
            {
                ...
                for (int j = 0; j < IControlService.CURRENT_DS4_CONTROLLER_LIMIT; j++)
                {
                ...
                for (int j = 0; j < IControlService.CURRENT_DS4_CONTROLLER_LIMIT; j++)
                {
                    if (Global.useTempProfile[j])
                    {
                        if (DS4Windows.Global.AutoProfileRevertDefaultProfile)
                        {
                            ...
                            Global.LoadProfile(j, false, _controlService);
                        }
                        else
                        {
                            if (autoProfileDebugLogLevel > 0)
    ...
    private bool GetTopWindowName(out string topProcessName, out string topWndTitleName)
    {
        ...
        else
        {
            ...
            hProcess = Kernel32.OpenProcess(0x0410, false, lpdwProcessId);
            if (hProcess != IntPtr.Zero) PsApi.GetModuleFileNameEx(hProcess, IntPtr.Zero, autoProfileCheckTextBuilder, autoProfileCheckTextBuilder.Capacity);
        ...
        User32.GetWindowText(hWnd, autoProfileCheckTextBuilder, autoProfileCheckTextBuilder.Capacity);
    ...
    private void SetAndWaitServiceStatus(bool serviceRunningStatus)
    {
        ...
        if (_controlService.IsRunning != serviceRunningStatus)
        {
            ...
            sw.Start();
            while (_controlService.IsRunning != serviceRunningStatus && sw.Elapsed.TotalSeconds < 10)
            {

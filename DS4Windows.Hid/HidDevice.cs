...
namespace DS4Windows
{
    public class HidDevice : IDisposable
    {
        ...
        {
            if (IsOpen)
                Kernel32.CancelIoEx(safeReadHandle.DangerousGetHandle(), IntPtr.Zero);
        }
        ...
        public bool ReadInputReport(byte[] data)
        {
            ...
            return Hid.HidD_GetInputReport(safeReadHandle, data, data.Length);
        }
        ...
        public bool WriteFeatureReport(byte[] data)
        {
            ...
            {
                result = Hid.HidD_SetFeature(safeReadHandle, data, data.Length);
            }
        ...
        {
            var deviceAttributes = default(Hid.HIDD_ATTRIBUTES);
            deviceAttributes.Size = Marshal.SizeOf(deviceAttributes);
            Hid.HidD_GetAttributes(hidHandle.DangerousGetHandle(), ref deviceAttributes);
        ...
        {
            var capabilities = default(Hid.HIDP_CAPS);
            var preparsedDataPointer = default(IntPtr);

            if (Hid.HidD_GetPreparsedData(hidHandle.DangerousGetHandle(), ref preparsedDataPointer))
            {
                Hid.HidP_GetCaps(preparsedDataPointer, ref capabilities);
                Hid.HidD_FreePreparsedData(preparsedDataPointer);
            }
        ...
        public void flush_Queue()
        {
            if (safeReadHandle != null)
            {
                Hid.HidD_FlushQueue(safeReadHandle);
            }
        }
        ...
        public ReadStatus ReadFile(byte[] inputBuffer)
        {
            ...
            {
                uint bytesRead;
                if (Kernel32.ReadFile(safeReadHandle.DangerousGetHandle(), inputBuffer, (uint)inputBuffer.Length, out bytesRead, IntPtr.Zero))
                {
                    return ReadStatus.Success;
                }
                else
        ...
        public bool WriteOutputReportViaControl(byte[] outputBuffer)
        {
            ...
            if (Hid.HidD_SetOutputReport(safeReadHandle, outputBuffer, outputBuffer.Length))
                return true;
            else
                return false;
        }
        ...
        private SafeFileHandle OpenHandle(String devicePathName, Boolean isExclusive, bool enumerate)
        {
            ...
            if (isExclusive)
            {
                hidHandle = Kernel32.CreateFile(devicePathName, access, 0, IntPtr.Zero, NativeMethods.OpenExisting, NativeMethods.FILE_FLAG_NO_BUFFERING | NativeMethods.FILE_FLAG_WRITE_THROUGH | NativeMethods.FILE_ATTRIBUTE_TEMPORARY | NativeMethods.FILE_FLAG_OVERLAPPED, 0);
            }
            else
            {
                hidHandle = Kernel32.CreateFile(devicePathName, access, NativeMethods.FILE_SHARE_READ | NativeMethods.FILE_SHARE_WRITE, IntPtr.Zero, NativeMethods.OpenExisting, NativeMethods.FILE_FLAG_NO_BUFFERING | NativeMethods.FILE_FLAG_WRITE_THROUGH | NativeMethods.FILE_ATTRIBUTE_TEMPORARY | NativeMethods.FILE_FLAG_OVERLAPPED, 0);
            }
        ...
        {
            return Hid.HidD_GetFeature(safeReadHandle.DangerousGetHandle(), inputBuffer, inputBuffer.Length);
        }
        ...
        public string ReadSerial(byte featureID = 18)
        {
            ...
            else
            {
                ...
                uint bufferLen = 126;
#endif
                if (Hid.HidD_GetSerialNumberString(safeReadHandle.DangerousGetHandle(), buffer, bufferLen))
                {

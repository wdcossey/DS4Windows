...
public class DS4Device
{
    ...
    private DS4ControllerOptions _nativeOptionsStore;
    ...
    public ConnectionType GetConnectionType() => conType;
    ...
    public int GetIdleTimeout() => idleTimeout;
    ...
    private int _rumbleAutoStopTime = 0;
    public int RumbleAutoStopTime
    {
        get => _rumbleAutoStopTime;
        set
        {
            // Value in milliseconds
            _rumbleAutoStopTime = value;
    ...
    public DS4Device(HidDevice hidDevice, string disName, VidPidFeatureSet featureSet = VidPidFeatureSet.DefaultDS4)
    {
        ...
        if (FeatureSet != VidPidFeatureSet.DefaultDS4)
            AppLogger.LogToGui($"The gamepad {displayName} ({conType}) uses custom feature set ({FeatureSet.ToString("F")})", false);
    ...
    public virtual void PostInit()
    {
        ...
        OptionsStore = _nativeOptionsStore = new DS4ControllerOptions(deviceType);
        SetupOptionsEvents();
    ...
    protected unsafe void performDs4Input()
    {
        unchecked
        {
            ...
            Hid.HidD_SetNumInputBuffers(hDevice.safeReadHandle.DangerousGetHandle(), 3);
            ...
            while (!exitInputThread)
            {
                ...
                if (conType == ConnectionType.BT && btInputReport[0] != 0x11 && (featureSet & VidPidFeatureSet.OnlyInputData0x01) == 0)
                {
    ...
    private void sendOutputReport(bool synchronous, bool force = false, bool quitOutputThreadOnError = true)
    {
        ...
        if (rumbleAutostopTimer.IsRunning)
        {
            ...
            if (rumbleAutostopTimer.ElapsedMilliseconds >= _rumbleAutoStopTime)
                SetRumble(0, 0);
        }
    ...
    public virtual bool DisconnectBT(bool callRemoval = false)
    {
        if (Mac != null)
        {
            ...
            lock (outputReport)
            {
                Bthprops.BLUETOOTH_FIND_RADIO_PARAMS p = new Bthprops.BLUETOOTH_FIND_RADIO_PARAMS();
                p.dwSize = Marshal.SizeOf(typeof(Bthprops.BLUETOOTH_FIND_RADIO_PARAMS));
                IntPtr searchHandle = Bthprops.BluetoothFindFirstRadio(ref p, ref btHandle);
                int bytesReturned = 0;

                while (!success && btHandle != IntPtr.Zero)
                {
                    success = Kernel32.DeviceIoControl(btHandle, IOCTL_BTH_DISCONNECT_DEVICE, ref lbtAddr, 8, IntPtr.Zero, 0, ref bytesReturned, IntPtr.Zero);
                    Kernel32.CloseHandle(btHandle);
                    if (!success)
                    {
                        if (!Bthprops.BluetoothFindNextRadio(searchHandle, ref btHandle))
                            btHandle = IntPtr.Zero;
                    }
                }

                Bthprops.BluetoothFindRadioClose(searchHandle);
    ...
    {
        if (_nativeOptionsStore != null)
        {
            if (_nativeOptionsStore.IsCopyCat)
            {
    ...
    private void SetupOptionsEvents()
    {
        if (_nativeOptionsStore != null)
        {
            _nativeOptionsStore.IsCopyCatChanged += (sender, e) =>
            {

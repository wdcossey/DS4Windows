...
namespace DS4Windows
{
    public class Mapping
    {
        ...
        public static OneEuroFilter[] wheelFilters = new OneEuroFilter[IControlService.MAX_DS4_CONTROLLER_COUNT];
        ...
        */
                        ...
                        if (triggeractivated)
                        {
                            ...
                            else if (action.typeID == SpecialAction.ActionTypeId.Profile)
                            {
                                ...
                                if (!actionDone[index].dev[device] && (!useTempProfile[device] || untriggeraction[device] == null || untriggeraction[device].typeID != SpecialAction.ActionTypeId.Profile) )
                                {
                                    ...
                                    var prolog = string.Format(Localization.UsingProfile, (device + 1).ToString(), action.details, $"{d.Battery}");
                ...
                if (utriggeractivated && action.typeID == SpecialAction.ActionTypeId.Profile)
                {
                    ...
                    action.controls != action.ucontrols)
                    {
                        if (useTempProfile[device])
                        {
                            ...
                            var profileName = untriggeraction[device].prevProfileName;
                            var d = ctrl.DS4Controllers[device];
                            var prolog = string.Format(Localization.UsingProfile,
        ...
        private static double getMouseMapping(int device, DS4Controls control, DS4State cState, DS4StateExposed eState,
            DS4StateFieldMapping fieldMapping, int mnum, IControlService ctrl)
        {
            int deadzoneL = 0;
            int deadzoneR = 0;

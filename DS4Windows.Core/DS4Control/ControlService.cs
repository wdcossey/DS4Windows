...
public interface IControlService
{
    ...
    #region Events
    event EventHandler<DebugEventArgs> Debug;
    public event EventHandler ServiceStarted;
    public event EventHandler PreServiceStop;
    public event EventHandler ServiceStopped;
    public event EventHandler RunningChanged;
    public event ControlService.HotplugControllerHandler HotplugController;
    #endregion
    
    DS4Device[] DS4Controllers { get; }
    bool IsRunning { get; }
    bool IsSuspending { get; set; }
    bool IsRecordingMacro { get; set; }
    Mouse[] TouchPad { get; }
    ControllerSlotManager SlotManager { get; }
    OutputDevice[] OutputDevices { get; }
    OutputSlotManager OutputslotMan { get; }
    ...
    void PrepareAbort();
    bool Start(bool showlog = true);
    bool Stop(bool showlog = true, bool immediateUnplug = false);
    bool HotPlug();
    void ShutDown();
    
    void ChangeUDPStatus(bool state, bool openPort = true);
    void ChangeOSCListenerStatus(bool state);
    void ChangeOSCSenderStatus(bool state);
    ...
    void LogDebug(String Data, bool warning = false);

    void SetRumble(byte heavyMotor, byte lightMotor, int deviceNum);
    void SetDevRumble(DS4Device device, byte heavyMotor, byte lightMotor, int deviceNum);
...
public class ControlService : IControlService
{
    ...
    public DS4Device[] DS4Controllers { get; } = new DS4Device[MAX_DS4_CONTROLLER_COUNT];
    
    public int activeControllers = 0;
    
    public Mouse[] TouchPad { get; } = new Mouse[MAX_DS4_CONTROLLER_COUNT];
    ...
    public ControllerSlotManager SlotManager { get; } = new();
    
    public bool IsRecordingMacro { get; set; } = false;
    ...
    public OutputDevice[] OutputDevices { get; } = new OutputDevice[MAX_DS4_CONTROLLER_COUNT] { null, null, null, null, null, null, null, null };
    ...
    private HashSet<string> hidDeviceHidingAffectedDevs = new();
    private HashSet<string> hidDeviceHidingExemptedDevs = new();
    ...
    public ControlServiceDeviceOptions DeviceOptions => deviceOptions;
    
    
    private readonly ArgumentParser _cmdParser;
    private readonly IDS4Devices _ds4Devices;
    ...
    private object busThrLck = new();
    private bool busThrRunning = false;
    private Queue<Action> busEvtQueue = new();
    private object busEvtQueueLock = new();
    
    public ControlService(IOptions<ArgumentParser> argsOptions, IDS4Devices ds4Devices)
    {
        _ds4Devices = ds4Devices;
        _cmdParser = argsOptions.Value;
    ...
    {
        string attemptVirtualkbmHandler = _cmdParser.VirtualkbmHandler;
    ...
    private void OutputslotMan_ViGEmFailure(object sender, EventArgs e)
    {
        eventDispatcher.BeginInvoke((Action)(() =>
        {
            ...
            LogDebug(Translations.Strings.ViGEmPluginFailure, true);
            Stop();
        }));
    }
    ...
    {
        IEnumerable<DS4Device> devices = _ds4Devices.GetDS4Controllers();
        if (state)
        {
            int i = 0;
    ...
    private void WarnExclusiveModeFailure(DS4Device device)
    {
        ...
        {
            string message = Localization.CouldNotOpenDS4.Replace("*Mac address*", device.GetMacAddress()) + " " +
                             Localization.QuitOtherPrograms;
    ...
    public bool Start(bool showlog = true)
    {
        ...
        {
            if (showlog)
                LogDebug(Localization.Starting);
            ...
            if (showlog)
            {
                LogDebug(Localization.SearchingController);
                LogDebug(DS4Devices.isExclusiveMode ? Localization.UsingExclusive : Localization.UsingShared);
            }
            ...
            try
            {
                     ...
                     devEnum.MoveNext() && loopControllers; i++)
                {
                    ...
                    if (i >= IControlService.CURRENT_DS4_CONTROLLER_LIMIT) // out of Xinput devices!
                        break;
                }
            }
            catch (Exception e)
    ...
    public bool Stop(bool showlog = true, bool immediateUnplug = false)
    {
        if (IsRunning)
        {
            ...
            if (showlog)
                LogDebug(Localization.StoppingX360);
            ...
            for (int i = 0, arlength = DS4Controllers.Length; i < arlength; i++)
            {
                ...
                if (tempDevice != null)
                {
                    if ((DCBTatStop && !tempDevice.IsCharging()) || IsSuspending)
                    {
                    ...
                    else
                    {
                        ...
                        tempDevice.StopUpdate();
                        _ds4Devices.RemoveDevice(tempDevice);
                        Thread.Sleep(50);
                    }
            ...
            if (showlog)
                LogDebug(Localization.StoppingDS4);

            _ds4Devices.StopControllers();
            ...
            if (showlog)
                LogDebug(Localization.StoppedDS4Windows);
        ...
        if (IsRunning)
        {
            ...
            for (var devEnum = devices.GetEnumerator(); devEnum.MoveNext() && loopControllers;)
            {
                ...
                for (int Index = 0, arlength = DS4Controllers.Length;
                     Index < arlength && Index < IControlService.CURRENT_DS4_CONTROLLER_LIMIT; Index++)
                {
                    if (DS4Controllers[Index] == null)
                    {
    ...
    private void PrepareConnectedInputControllerSettingEvents(int numControllers, DS4Device device, int index)
    {
        ...
        device.Removal += _ds4Devices.On_Removal;
        device.SyncChange += this.On_SyncChange;
        device.SyncChange += _ds4Devices.UpdateSerial;
    ...
    private void BeginPrepareConnectedInputController(DS4Device device, bool showlog = false)
    {
        if (showlog)
        {
            LogDebug($"{Localization.FoundController} {device.GetMacAddress()} ({device.GetConnectionType()}) ({device.DisplayName})");
        }
    ...
    protected void On_SyncChange(object sender, EventArgs e)
    {
        DS4Device device = (DS4Device)sender;
        int ind = -1;
        for (int i = 0, arlength = IControlService.CURRENT_DS4_CONTROLLER_LIMIT; ind == -1 && i < arlength; i++)
        {
            DS4Device tempDev = DS4Controllers[i];
            if (tempDev != null && device == tempDev)
                ind = i;
    ...
    protected void On_DS4Removal(object sender, EventArgs e)
    {
        ...
        if (ind != -1)
        {
            ...
            if (removingStatus)
            {
                ...
                var removed = Localization.ControllerWasRemoved.Replace("*Mac address*", (ind + 1).ToString());
                if (device.GetBattery() <= 20 &&
                    device.GetConnectionType() == ConnectionType.BT && !device.IsCharging())
                {
                    removed += ". " + Localization.ChargeController;
                }
        ...
        if (ind != -1)
        {
            ...
            if (device.firstReport && device.isSynced())
            {
                ...
                if (device.PrimaryDevice)
                {
                    ...
                    {
                        var prolog = string.Format(Localization.UsingProfile, (ind + 1).ToString(), ProfilePath[ind], $"{device.Battery}");
                        LogDebug(prolog);
                        AppLogger.LogToTray(prolog);
                    }
                    else
                    {
                        var prolog = string.Format(Localization.NotUsingProfile, (ind + 1).ToString(), $"{device.Battery}");
                        LogDebug(prolog);
                        AppLogger.LogToTray(prolog);
                    }
                }
            ...
            if (!IsRecordingMacro && (useTempProfile[ind] ||
            ...
            else
            {
                ...
                switch (steeringWheelMappedAxis)
                {
                    ...
                    case SASteeringWheelEmulationAxisType.VJoy2X:
                        vJoyFeeder.FeedAxisValue(cState.SASteeringWheelEmulationUnit, ((((uint)steeringWheelMappedAxis) - ((uint)SASteeringWheelEmulationAxisType.VJoy1X)) / 3) + 1, VJoyFeeder.HID_USAGES.HID_USAGE_X);
                        break;

                    case SASteeringWheelEmulationAxisType.VJoy1Y:
                    case SASteeringWheelEmulationAxisType.VJoy2Y:
                        vJoyFeeder.FeedAxisValue(cState.SASteeringWheelEmulationUnit, ((((uint)steeringWheelMappedAxis) - ((uint)SASteeringWheelEmulationAxisType.VJoy1X)) / 3) + 1, VJoyFeeder.HID_USAGES.HID_USAGE_Y);
                        break;

                    case SASteeringWheelEmulationAxisType.VJoy1Z:
                    case SASteeringWheelEmulationAxisType.VJoy2Z:
                        vJoyFeeder.FeedAxisValue(cState.SASteeringWheelEmulationUnit, ((((uint)steeringWheelMappedAxis) - ((uint)SASteeringWheelEmulationAxisType.VJoy1X)) / 3) + 1, VJoyFeeder.HID_USAGES.HID_USAGE_Z);
                        break;
    ...
    private void LagFlashWarning(DS4Device device, int ind, bool on)
    {
        ...
        {
            lag[ind] = true;
            LogDebug(string.Format(Localization.LatencyOverTen, (ind + 1), device.Latency), true);
            if (getFlashWhenLate())
            {
        ...
        {
            lag[ind] = false;
            LogDebug(Localization.LatencyNotOverTen.Replace("*number*", (ind + 1).ToString()));
    ...
    protected void CheckForTouchToggle(int deviceID, DS4State cState, DS4State pState)
    {
        if (!IsUsingTouchpadForControls(deviceID) && cState.Touch1 && pState.PS)
        {
            if (GetTouchActive(deviceID) && touchreleased[deviceID])
            {
                ...
                LogDebug(Localization.TouchpadMovementOff);
                AppLogger.LogToTray(Localization.TouchpadMovementOff);
                touchreleased[deviceID] = false;
            }
            else if (touchreleased[deviceID])
            {
                TouchActive[deviceID] = true;
                LogDebug(Localization.TouchpadMovementOn);
                AppLogger.LogToTray(Localization.TouchpadMovementOn);
                touchreleased[deviceID] = false;
            }
        }
        else
    ...
    {
        if (deviceID < IControlService.CURRENT_DS4_CONTROLLER_LIMIT)
        {
            TouchActive[deviceID] = false;
        }
    }
